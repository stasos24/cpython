...
//Iniatilisation xml.parser.expat.parse
PyObject* xml_module = NULL;
PyObject* xml_object = NULL;
/* Called by LLVMFuzzerTestOneInput for initialization */
static int init_xml_parse(void) {
...
#endif
#if !defined(_Py_FUZZ_ONE) || defined(_Py_FUZZ_fuzz_xml_parse)
    static int XML_INITIALIZED = 0;
    if (!XML_INITIALIZED && !init_xml_parse()) {
        PyErr_Print();
        abort();
    } else {
        XML_INITIALIZED = 1;
    }
    rv |= _run_fuzz(data, size, fuzz_xml_parse);
#endif
  return rv;
}
